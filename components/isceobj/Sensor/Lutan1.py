...
XML = Component.Parameter(
    "xml", public_name="xml", default=None, type=str, doc="Input XML file"
)


TIFF = Component.Parameter(
    "tiff", public_name="tiff", default=None, type=str, doc="Input image file"
)

ORBIT_FILE = Component.Parameter(
    "orbitFile", public_name="orbitFile", default=None, type=str, doc="Orbit file"
)
...
class Lutan1(Sensor):
    ...
    def _resolve_inputs_from_safe(self):
        ...
        if safe.lower().endswith(".zip"):
            ...
            tiffs = sorted(
                [m for m in members if (not m.endswith("/")) and self._is_tiff_name(m)]
            )
            if len(tiffs) == 0:
            ...
            if self.xml is None:
                if chosen_xml is None:
                    raise Exception(
                        "No matching .meta.xml found for TIFF {} in zip {}".format(
                            chosen_tiff, safe
                        )
                    )
        ...
        if os.path.isdir(safe):
            ...
            tiffs += sorted(
                glob.glob(os.path.join(safe, "**", "*.tiff"), recursive=True)
            )
            if len(tiffs) == 0:
            ...
            if self.xml is None:
                if chosen_xml is None:
                    raise Exception(
                        "No matching .meta.xml found for TIFF {} in directory {}".format(
                            chosen_tiff, safe
                        )
                    )
                self.xml = chosen_xml
    ...
    def parse(self):
        ...
        if not hasattr(self, "safe"):
            self.safe = None
        if not hasattr(self, "orbitFilter"):
            self.orbitFilter = True
        if not hasattr(self, "orbitFilterDegree"):
            self.orbitFilterDegree = 5
        if not hasattr(self, "orbitFilterSigma"):
            self.orbitFilterSigma = 4.0
        if not hasattr(self, "orbitFilterMaxIter"):
            self.orbitFilterMaxIter = 3
        if not hasattr(self, "orbitFilterIgnoreStart"):
            self.orbitFilterIgnoreStart = -1
        if not hasattr(self, "orbitFilterIgnoreEnd"):
            self.orbitFilterIgnoreEnd = -1
        ...
        if self.xml is None:
            ...
            if tifExt.lower() not in [".tif", ".tiff"]:
                raise Exception(
                    "Unexpected TIFF extension for Lutan1 input: {}".format(self.tiff)
                )
        ...
        if self.orbitFile:
            ...
            warnings.warn(
                "WARNING! No orbit file found. Orbit information from the annotation file is used for processing."
            )
            orb = self.extractOrbitFromAnnotation()
        ...
        if UnivariateSpline is not None and tx.size >= deg + 2:
            try:
                ...
                if txu.size >= deg + 2:
                    rough = np.diff(yyu)
                    if rough.size > 1:
                        noise = self._robustScale(rough) * np.sqrt(2.0)
                    else:
                        noise = self._robustScale(yyu)
    ...
    def _filterOrbit(self, orb):
        ...
        if nvec < min_needed:
            warnings.warn(
                "Lutan1 orbit filter skipped: only {} state vectors (need >= {}).".format(
                    nvec, min_needed
                )
            )
            return vecs
    ...
    def populateMetadata(self):
        ...
        polarization = self.grab_from_xml(
            "productInfo/acquisitionInfo/polarisationMode"
        )
        frequency = float(
            self.grab_from_xml("instrument/radarParameters/centerFrequency")
        )
        passDirection = self.grab_from_xml("productInfo/missionInfo/orbitDirection")
        rangePixelSize = float(
            self.grab_from_xml("productInfo/imageDataInfo/imageRaster/columnSpacing")
        )
        azimuthPixelSize = float(
            self.grab_from_xml("productInfo/imageDataInfo/imageRaster/rowSpacing")
        )
        rangeSamplingRate = Const.c / (2.0 * rangePixelSize)

        prf = float(self.grab_from_xml("instrument/settings/settingRecord/PRF"))
        lines = int(
            self.grab_from_xml("productInfo/imageDataInfo/imageRaster/numberOfRows")
        )
        samples = int(
            self.grab_from_xml("productInfo/imageDataInfo/imageRaster/numberOfColumns")
        )
        ...
        if crop_far > 0:
            ...
            print(
                "Lutan1: cropping far-range by {} pixels, effective samples={}".format(
                    crop_far, samples
                )
            )

        startingRange = (
            float(self.grab_from_xml("productInfo/sceneInfo/rangeTime/firstPixel"))
            * Const.c
            / 2.0
        )
        # slantRange = float(self.grab_from_xml('productSpecific/complexImageInfo/'))
        incidenceAngle = float(
            self.grab_from_xml("productInfo/sceneInfo/sceneCenterCoord/incidenceAngle")
        )
        dataStartTime = self.convertToDateTime(
            self.grab_from_xml("productInfo/sceneInfo/start/timeUTC")
        )
        dataStopTime = self.convertToDateTime(
            self.grab_from_xml("productInfo/sceneInfo/stop/timeUTC")
        )
        pulseLength = float(
            self.grab_from_xml(
        ...
        processingFacility = self.grab_from_xml(
    ...
    def extractOrbit(self):
        ...
        # Use 60s margin to provide sufficient state vectors for
        # Hermite (4pts) and Legendre (9pts) orbit interpolation,
        # avoiding edge artifacts in geo2rdr azimuth offsets.
        margin = datetime.timedelta(seconds=60.0)
    ...
    def extractOrbitFromAnnotation(self):
        ...
        for k in range(1, countNode):
            timestamp = self.convertToDateTime(
                node.find("stateVec[{}]/timeUTC".format(k)).text
            )
            if (timestamp >= tstart) and (timestamp <= tend):
                pos = [
                    float(node.find("stateVec[{}]/posX".format(k)).text),
                    float(node.find("stateVec[{}]/posY".format(k)).text),
                    float(node.find("stateVec[{}]/posZ".format(k)).text),
                ]
                vel = [
                    float(node.find("stateVec[{}]/velX".format(k)).text),
    ...
    def extractDoppler(self):
        ...
        if dc_root is None:
            raise Exception(
                "Could not find processing/doppler/dopplerCentroid in Lutan1 metadata."
            )
        ...
        if len(estimates) == 0:
            raise Exception(
                "Could not find dopplerEstimate records in Lutan1 metadata."
            )
        ...
        for est in estimates:
            ...
            if etime is None:
                ...
                try:
                    delta = abs(
                        (
                            self.convertToDateTime(etime.strip())
        ...
        if dpoly is None:
            raise Exception(
                "Could not extract valid Doppler polynomial from Lutan1 metadata."
            )
        ...
        rngs = (
            self.frame.startingRange
            + pix * self.frame.getInstrument().getRangePixelSize()
        )
        evals = dpoly(rngs)
        ...
        self.doppler_coeff = (
            list(selected_coeffs) if selected_coeffs is not None else None
        )
        print(
            "Average doppler quadratic(a, b, c): ",
            quadratic["a"],
            quadratic["b"],
            quadratic["c"],
        )
