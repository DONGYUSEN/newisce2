...
def createUnwrap2Stage(other, do_unwrap_2stage=None, unwrapperName=None):
    if (
        (not do_unwrap_2stage)
        or (unwrapperName.lower() == "icu")
        or (unwrapperName.lower() == "grass")
    ):
        # if not defined create an empty method that does nothing
        ...
        try:
            ...
            raise Exception(
                "Please install PuLP Linear Programming API to run 2stage unwrap"
            )
...
createRefocusBursts = _factory("runRefocusBursts", "runRefocusBursts")
