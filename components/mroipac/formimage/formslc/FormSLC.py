...
START_RANGE_BIN = Component.Parameter(
    ...
    mandatory=False,
    doc=(
        "Starting range bin to read from the raw data. " + "Must have positive value."
    ),
)
NUMBER_RANGE_BIN = Component.Parameter(
    ...
    mandatory=True,
    doc=(
        "Number of range bins in the input raw image. "
...
TERRAIN_HEIGHT = Component.Parameter(
    "terrainHeight",
    public_name="TERRAIN_HEIGHT",
    default=[],
    type=list,
    mandatory=False,
    doc="Per-range-bin terrain height array (m) for terrain-dependent RCMC",
)
USE_TERRAIN_FLAG = Component.Parameter(
    "useTerrainFlag",
    public_name="USE_TERRAIN_FLAG",
    default=False,
    type=bool,
    mandatory=False,
    doc="Flag to enable terrain-dependent RCMC correction",
)
...
class FormSLC(Component):
    ...
    def getState(self):
        ...
        if outStart != self.startingRange:
            raise Exception(
                "Starting Range mismatch: {0} {1}".format(outStart, self.startingRange)
            )

        deltat = formslc.getSLCStartingLine_Py()
        self.slcSensingStart = self.sensingStart + datetime.timedelta(
            seconds=deltat / self.prf
        )
        return
    ...
    def setState(self):
        ...
        if self.useTerrainFlag and self.terrainHeight:
            formslc.setUseTerrainFlag_Py(int(self.useTerrainFlag))
            formslc.setTerrainHeight_Py(self.terrainHeight)
    ...
    def checkInitialization(self):
        ...
        slcWidth = (
            int((self.numberGoodBytes - self.firstSample) / 2)
            + nearRangeExt
            - farRangeExt
            - pulseSamples
        )
        ...
        synthApertureSamps = (
            self.radarWavelength
            * (
                slcStartingRange
                + slcWidth * SPEED_OF_LIGHT * 0.5 / self.rangeSamplingRate
            )
            * self.prf
            / (self.antennaLength * self.bodyFixedVelocity)
        )
        ...
        if self.azimuthPatchSize and self.numberValidPulses:
            if (
                self.azimuthPatchSize < self.numberValidPulses
                or self.azimuthPatchSize < chunkedSAS + chunksize
            ):
                self.azimuthPatchSize = azP
                self.numberValidPulses = nV
            elif self.numberValidPulses > self.azimuthPatchSize - chunkedSAS:
                msg = (
                    "Number of valid pulses specified is too large "
                    + "for full linear convolution. "
                )
                msg += "Should be less than %d" % (self.azimuthPatchSize - chunkedSAS)
                self.logger.info(msg)
                self.logger.info(
                    "Continuing with specified value of %d" % self.numberValidPulses
                )
            ...
            self.azimuthPatchSize = 2 ** self.nxPower(
                self.numberValidPulses + synthApertureSamps
            )
            if self.azimuthPatchSize > self.maxAzPatchSize:
                msg = "%d is a rather large patch size. " % self.azimuthPatchSize
                msg += (
                    "Check that the number of valid pulses is in a "
    ...
    def setTerrainHeight(self, var):
        self.terrainHeight = var
        return

    def setUseTerrainFlag(self, var):
        self.useTerrainFlag = bool(var)
        return
