...
def _autodetect_wavelength_for_postprocess(insar_obj):
    ...
    for xmlname in xml_candidates:
        ...
        try:
            inst = (
                prod.getInstrument()
                if hasattr(prod, "getInstrument")
                else getattr(prod, "instrument", None)
            )
        except Exception:
            inst = None
        if inst is None:
...
FOCUSER = Application.Parameter(
    "focuser",
    public_name="focuser",
    default="formslc",
    type=str,
    mandatory=False,
    doc="SAR focusing method: formslc (range-Doppler via ROI) or backproject (time-domain backprojection via ISCE3 engine)",
)
...
class _RoiBase(Application, FrameMixin):
    ...
    def _configure(self):
        ...
        self.enableRdrdemOffsetLoop = bool(
            getattr(self, "enableRdrdemOffsetLoop", True)
        )
    ...
    def _add_methods(self):
        ...
        if getattr(self, "focuser", "formslc").lower() == "backproject":
            self.runFormSLC = StripmapProc.createFormSLCBackproject(self)
        else:
        ...
        self.runNormalizeSecondarySampling = (
            StripmapProc.createNormalizeSecondarySampling(self)
        )
        ...
        self.runRubbersheetRange = StripmapProc.createRubbersheetRange(
            self
        )  # Modified by V. Brancato 10.07.2019
        self.runRubbersheetAzimuth = StripmapProc.createRubbersheetAzimuth(
            self
        )  # Modified by V. Brancato 10.07.2019
    ...
    def _steps(self):
        ...
        self.step(
            "normalize_secondary_sampling", func=self.runNormalizeSecondarySampling
        )
        ...
        if self.doDispersive:
            self.step(
                "sub_band_interferogram", func=self.runInterferogram, args=("sub",)
            )
            self.step(
                "filter_low_band",
                func=self.runFilter,
        ...
        self.step(
            "geocode", func=self.runGeocode, args=(self.geocode_list, self.geocode_bbox)
        )
